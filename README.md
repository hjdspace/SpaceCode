...
完整的压缩工具与插件生态系统，支持多平台集成：

- **核心压缩模式**：支持多种压缩算法和策略
- **多平台插件**：Claude Code、OpenCode 等插件集成
- **配套工具**：代码审查、提交信息生成等 AI 工具
- **完整文档**：提供使用指南、API 文档和最佳实践

##### 📋 Manus 风格文件式规划

支持多 IDE 和多语言的文件式规划技能套件：

- **多 IDE 支持**：Claude Code、Cursor、GitHub Copilot 等
- **多语言命令**：阿拉伯语、德语、西班牙语、中文
- **完整流程**：基础配置、IDE 钩子、测试用例、CI 流程
...
- **极简主义** (Minimalist UI)：干净编辑器风格界面
- **工业粗野主义** (Industrial Brutalist)：机械终端美学
- **高端视觉设计** (High-End Visual Design)： Agency 级别设计规范
- **品牌套件生成** (BrandKit)： premium 品牌识别系统
- **Stitch 设计规范**：Google Stitch 专用设计系统
- **版本锁定**：skills-lock.json 确保技能版本一致性
