...
[多模式核心](#多模式核心) · [会话编排](#-会话编排sleep-coding) · [Computer Use](#-computer-use桌面控制) · [Browser Use](#-browser-use浏览器自动化) · [桌面宠物](#-桌面宠物desktop-pet) · [移动端](#-移动端配套) · [更多特色](#更多特色) · [快速开始](#快速开始) · [项目结构](#项目结构) · [开发指南](#开发指南) · [技术栈](#技术栈)
...
把一串**互相有依赖**的任务画成一张有向图，睡前点一次运行，机器替你整夜接力。

**为什么需要它：** 真实的工作流常常是「做完 A 才能做 B，B 完成后才能做 C」，有时还有一
堆可以并行开跑的支路。过去你只能守在屏幕前，等一个会话结束再手动开下一个会话、把上下文
重新交代一遍。会话编排把这条链路搬到画布上：上游会话的 turn 一结束，下游节点自动点火，
失败的支路不拖累旁支 —— **你睡觉的时候，流水线在跑**（sleep coding）。

**核心亮点：**

| 能力 | 说明 |
|------|------|
| **依赖即连线** | 画布上从节点边缘拖出连线，表达「等它完成再开始」；一条上游可分出多条并行支路，多条上游也可汇聚成「全部完成才启动」，成环的连线会被当场拒绝 |
| **一次性隔离会话** | 每次运行、每个任务节点都创建一个全新且上下文隔离的真实会话，跑完照常出现在侧边栏，可回到任一节点继续追问 |
| **失败不拖累旁支** | 节点失败或被手动停止 → 它的下游自动标记为「已跳过」，无关并行支路继续跑；一键重试失败节点，被跳过的下游依序自动续跑 |
| **运行中可读可控** | 节点内嵌缩略聊天窗，实时显示流式输出；小窗口内直接滚动即可看完整内容，双击放大为画布内抽屉继续交互；运行中可追加消息、单独停止某节点或整体停止 |
| **连线可视化进度** | 连线按上游状态着色并带流动动效：待触发（灰）→ 运行中（靛蓝流动光点）→ 已完成（绿）→ 断裂（红虚线），整图依赖与进度一眼可读 |
| **编排不锁布局** | 运行中禁止增删节点与连线以保持图状态一致，但**节点位置随时可拖拽重排**，方便边跑边整理视图 |
...
1. 白天用 `to-issues` 把需求拆成若干有依赖的切片，逐个写进节点的草稿（草稿不会发送，刷新也不丢）
2. 连线：`1 → 2 → 3` 串行，`3` 再分出 `4a`、`4b` 并行，`5` 汇聚 `4a`、`4b`
3. 睡前点「运行」：零依赖节点并行点火，每个节点跑在全新会话里，上游结束自动触发下游
4. 早上回来读整图：绿=完成、红=失败、灰线=未触发；失败节点一键重试，它的下游自动续跑
...
│   │   ├── work/               # 工作模式与办公助手
│   │   ├── debug/              # 链路追踪
│   │   ├── orchestration/      # 会话编排画布（Task Node / 依赖连线 / 节点抽屉）
