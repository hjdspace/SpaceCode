...
- 运行中锁结构：禁止增删节点和连线；未开始节点的草稿仍可编辑。**节点位置/布局不在锁的范围内** —— 移动节点不改变 DAG 结构，运行中同样可拖拽重排（见 CONTEXT.md 的 Structure Lock）。
  执行细节：`isRunning` 由引擎真实运行态推导，而非手工维护的布尔量 —— retry / rerun 这类二次启动路径必须同样能释放锁。
...
- 节点卡片：自定义 Vue Flow 节点组件，内嵌真实 ChatPanel（传 sessionId prop，CSS transform 缩小）。节点头部显示状态标识 + 权限提示徽标，头部同时是拖拽手柄。
- 节点卡片内的缩略聊天窗必须自带滚动：小尺寸下内容超出时可直接在节点内滚动查看完整会话，不必先双击放大。
- 连线：自定义 Vue Flow Edge 组件。按上游节点状态着色（待触发 / 运行中 / 已完成 / 断裂），运行中的连线带流动高亮与流动光点，把整图依赖方向与进度做成可读的视觉语言。
