...
You have the **`xlsx` skill** bound. Read its `SKILL.md` first and follow its financial-model conventions. Save to `outputs/<name>.xlsx`.

## Standards (from the xlsx skill)
...
3. Generate the `.xlsx` via the skill; validate **zero formula errors**; report path.
