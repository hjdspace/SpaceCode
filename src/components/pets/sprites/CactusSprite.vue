...
<template>
  <svg viewBox="0 0 100 60" width="80" height="48" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <!-- 身体径向渐变：模拟柱体光影 -->
      <radialGradient id="cactusBodyGrad" cx="35%" cy="40%" r="80%">
        <stop offset="0%" stop-color="rgba(255,255,255,0.4)" />
        <stop offset="50%" stop-color="rgba(255,255,255,0)" />
        <stop offset="100%" stop-color="rgba(0,0,0,0.35)" />
      </radialGradient>
      <!-- 接地阴影模糊 -->
      <filter id="cactusShadowFilter" x="-50%" y="-50%" width="200%" height="200%">
        <feGaussianBlur stdDeviation="1.6" />
      </filter>
      <!-- 花朵光晕 -->
      <filter id="cactusFlowerGlow" x="-50%" y="-50%" width="200%" height="200%">
        <feGaussianBlur stdDeviation="0.8" />
      </filter>
    </defs>

    <!-- 接地阴影（不随跳跃移动） -->
    <ellipse cx="50" cy="55" rx="28" ry="2.5" fill="rgba(0,0,0,0.22)" filter="url(#cactusShadowFilter)" />
    ...
    <g :transform="`translate(0, ${bodyOffsetY})`">
      ...
      <g :transform="flowerTransform">
        <!-- 花瓣光晕 -->
        <circle cx="50" cy="9" r="4" :fill="palette.accent" opacity="0.35" filter="url(#cactusFlowerGlow)" />
        <!-- 花瓣（4 片） -->
