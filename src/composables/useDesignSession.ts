...
import { useChatStore } from '@/stores/chat'; // 现有 SpaceCode 聊天 Store 引用
...
export function useDesignSession() {
  ...
  async function startDesignGeneration() {
    ...
    try {
      ...
      const workspacePath = `${api.app.getPath('userData')}/design-workspace/${Date.now()}`;
      ...
      await chatStore.sendMessage(session.id, designStore.brief);
  ...
  function listenToStream(sessionId: string) {
    ...
    api.claudeCode.onStreamEvent(({ data, sessionId: eventSessionId }) => {
      ...
      if (data.type === 'text_delta') {
        ...
        if (form && !designStore.pendingQuestionForm) {
          ...
          // 在前端 UI 聊天历史中更新此消息
          chatStore.updateLastAssistantMessage(sessionId, textOnly);
        }
  ...
  async function submitQuestionForm(answers: Record<string, any>) {
    ...
    await chatStore.sendMessage(sessionId, responseMessage);
  }
  ...
  async function stopDesignGeneration() {
    ...
    await chatStore.terminateSession(sessionId);
