---
name: berkshire-thesis-drift
description: "AI Berkshire skill: 投资论文漂移检测：分清事实变化与措辞变化. Source: berkshire-thesis-drift."
---

## Codex adapter note

This skill is generated from `berkshire-thesis-drift` so Claude Code and Codex users share one canonical workflow.
...
所有数值变化必须使用 `.claude/skills/berkshire-shared/tools/financial_rigor.py` 做精确计算，禁止 LLM 心算：

```bash
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py verify-valuation \
  --price {当前价格} \
  --eps {EPS} \
  --bvps {每股净资产} \
...
```bash
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py verify-market-cap --price {价格} --shares {股本} --reported {报告市值} --currency {币种}
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py cross-validate --field {字段} --values '{JSON}' --unit {单位}
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py three-scenario --price {价格} --eps {EPS} --shares {股本亿} --growth {乐观} {中性} {悲观} --pe {乐观PE} {中性PE} {悲观PE}
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py calc --expr '{精确算式}'
```
...
- **数值必须验算** — 所有百分比、估值倍数、目标价差异必须用 `.claude/skills/berkshire-shared/tools/financial_rigor.py`
