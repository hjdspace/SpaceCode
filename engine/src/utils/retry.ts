...
// 超时配置常量
export const DEFAULT_API_TIMEOUT_MS = 600 * 1000 // 10分钟 - 足够处理复杂请求
...
export function isRetryableError(error: any): boolean {
  ...
  // 检查自定义重试标志
  if ((error as RetryableError).isRetryable !== undefined) {
...
export async function withRetryAndTimeout<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!isRetryableError(error)) {
        logForDebugging(`[Retry] 错误不可重试: ${(error as Error).message}`)
        throw error
      }
  ...
  // 所有重试都失败
  logForDebugging(`[Retry] 所有 ${maxRetries + 1} 次尝试都失败了`)
  throw lastError
}

// 获取 API 超时配置
export function getApiTimeoutMs(): number {
  const envValue = process.env.API_TIMEOUT_MS
  if (envValue) {
...
// 创建可重试错误
export function createRetryableError(message: string, originalError?: any): Error & RetryableError {
  const error = new Error(message) as Error & RetryableError
