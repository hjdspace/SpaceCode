...
description: "AI Berkshire skill: 组合管理：从\"研究公司\"到\"管理组合\". Source: berkshire-portfolio-review."
---

## Codex adapter note

This skill is generated from `berkshire-portfolio-review` so Claude Code and Codex users share one canonical workflow.
...
对每个持仓使用 `.claude/skills/berkshire-shared/tools/financial_rigor.py verify-valuation` 校验估值数据。对每只持仓标注信息丰富度（A/B/C级），C级持仓的分析结论标注低置信度。
...
预期回报估算方法（使用 `.claude/skills/berkshire-shared/tools/financial_rigor.py three-scenario` 计算）：
