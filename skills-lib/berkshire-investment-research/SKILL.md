---
name: berkshire-investment-research
description: "AI Berkshire skill: 投资研究：巴菲特-芒格-段永平-李录 四大师综合分析框架. Source: berkshire-investment-research."
---

## Codex adapter note

This skill is generated from `berkshire-investment-research` so Claude Code and Codex users share one canonical workflow.
...
> **数据源规范**：参见 `berkshire-financial-data`。所有财务数据必须来自两个独立来源，误差>1%须标记。
...
数据收集完成后，**必须调用 `.claude/skills/berkshire-shared/tools/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
...
```bash
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py verify-market-cap \
  --price {股价} --shares {总股本} --reported {报告市值} --currency {币种}
```

Step 2 — 关键数据多源交叉验证：
```bash
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py cross-validate \
...
```bash
python3 .claude/skills/berkshire-shared/tools/financial_rigor.py three-scenario \
...
**Step 2 — 取数核验：**
对清单中每个数据点，按 `berkshire-financial-data` 规范从可靠信源取数
