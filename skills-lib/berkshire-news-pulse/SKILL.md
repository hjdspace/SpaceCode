---
name: berkshire-news-pulse
description: 公司新闻脉搏：股价异动时快速归因。用 4 个并行 Agent 侦察公司事件/监管政策/行业对手/市场情绪，产出"事件时间线 + 异动主因判断 + 是否触发论文重审"。
---

## Codex adapter note

This skill is generated from `berkshire-news-pulse` so Claude Code and Codex users share one canonical workflow.
...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 .claude/skills/berkshire-shared/tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     - 段永平 user_id: `1247347556`
     - 命令示例：`python3 .claude/skills/berkshire-shared/tools/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/dyp-{公司名}.md`
